...
def paired(iter):
    res = []
    for i, j in zip(range(0, len(iter), 2), range(1, len(iter), 2)):
        res.append((iter[i], iter[j]))
    return res
...
class ClassMethod(NamedTuple):
    ...
    return_type: str
    # body: str
    parameters_types: Tuple[str, str]

    javadocstr: str

    inferred_body: str = ""
    annotations: Union[Tuple[str, str], None] = None
...
def parse_method(method_node) -> ClassMethod:
    ...
    return ClassMethod(method_name, signature_parts[0], "static" in signature_parts, signature_parts[-2], parameters, field_desc)

def parse_from_html(html_obj) -> JavaDoc:
    ...
    return JavaDoc(class_name, new_class_methods, class_fields)

def gen_stub(jdoc: JavaDoc) -> str:
    text = f"public class {jdoc.class_name} {{ \n"
